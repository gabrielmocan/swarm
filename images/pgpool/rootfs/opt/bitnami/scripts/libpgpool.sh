...
export PGPOOL_CONNECTION_CACHE="${PGPOOL_CONNECTION_CACHE:-yes}"
export PGPOOL_BACKEND_FAILOVER="${PGPOOL_BACKEND_FAILOVER:-no}"
export PGPOOL_DETACH_FALSE_PRIMARY="${PGPOOL_DETACH_FALSE_PRIMARY:-no}"
...
pgpool_validate() {
    ...
    # check if connection cache is enabled
    if ! is_yes_no_value "$PGPOOL_CONNECTION_CACHE"; then
        print_validation_error "connection cache should be yes or no"
    fi

    # check if backend failover is enabled
    if ! is_yes_no_value "$PGPOOL_BACKEND_FAILOVER"; then
        print_validation_error "backend failover should be yes or no"
    fi

    # check if detach false primary is enabled
    if ! is_yes_no_value "$PGPOOL_DETACH_FALSE_PRIMARY"; then
        print_validation_error "Detach false primary should be yes or no"
    fi
...
pgpool_create_config() {
    ...
    [[ -n "${PGPOOL_CONNECTION_CACHE:-}" ]] && pgpool_set_property "connection_cache" "$PGPOOL_CONNECTION_CACHE"
    ...
    pgpool_set_property "failover_on_backend_error" "$PGPOOL_BACKEND_FAILOVER"
    pgpool_set_property "detach_false_primary" "$PGPOOL_DETACH_FALSE_PRIMARY"
